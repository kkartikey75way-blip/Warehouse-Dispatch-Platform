...
export const startDeliveryService = async (shipmentId: string, driverId: string) => {
    const shipment = await findShipmentById(shipmentId);

    if (!shipment) throw new AppError("Shipment not found", 404);
    if (!shipment.acceptedByDriver) throw new AppError("Shipment must be accepted before starting delivery", 400);
...
export const markInTransitService = async (shipmentId: string) => {
    const delivery = await updateDeliveryStatus(shipmentId, ShipmentStatus.IN_TRANSIT);
    if (!delivery) throw new AppError("Delivery not found", 404);
    return delivery;
};

export const completeDeliveryService = async (shipmentId: string, signatureUrl: string, photoUrl: string) => {
    const delivery = await addProofOfDelivery(shipmentId, signatureUrl, photoUrl);
    if (!delivery) throw new AppError("Delivery not found", 404);

    const updatedShipment = await updateShipmentRepoStatus(shipmentId, ShipmentStatus.DELIVERED);
    if (!updatedShipment) throw new AppError(`Failed to update shipment status for ${shipmentId}`, 500);
...
export const reportExceptionService = async (shipmentId: string, exception: ExceptionType) => {
    const delivery = await addDeliveryException(shipmentId, exception);
    if (!delivery) throw new AppError("Delivery not found", 404);
